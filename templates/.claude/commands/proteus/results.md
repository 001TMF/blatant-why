...
If argument provided, look up campaign by ID in `.proteus/campaigns/`.
Otherwise, use the active campaign from `.proteus/active_campaign`.
...
### Step 3: Spawn proteus-verifier agent

Delegate to a **proteus-verifier** agent (model per profile table above):
