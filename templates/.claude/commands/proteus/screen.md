...
Run the full Proteus screening battery on a set of designs.
...
If no argument, use the active campaign from `.proteus/active_campaign`.
...
### Step 3: Spawn proteus-screening agent

Delegate to a **proteus-screening** agent (model per profile table above) with instructions:
