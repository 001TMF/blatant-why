...
```bash
mkdir -p .proteus
if [ ! -f .proteus/config.json ]; then
  echo '{"model_profile": "balanced"}' > .proteus/config.json
fi
```

### Step 2: Spawn proteus-environment agent

Delegate to a **proteus-environment** agent (model per profile table above):

> Scan the compute environment and produce a complete inventory.
>
> **Local tools** — check these paths and report version if found:
> - proteus-fold (Protenix): `/data/proteus/Protenix/`
> - proteus-prot (PXDesign): `/data/proteus/PXDesign/`
> - proteus-ab (Proteus-AB): `/data/proteus/proteus-design/`
>
...
>
> Write complete results to `.proteus/environment.json`.
